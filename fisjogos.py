...
class Snake:
    ...
    def check_apple(self):
        ...
        if self.snake[0] == self.apple:
            score = 1 if not self.super_apple else SUPER_APPLE_POINTS
            self.points_to_append = score
        ...
        if self.points_to_append > 0:
            self.snake.append(self.popped_point)
            self.points_to_append -= 1
    ...
    def generate_apple(self):
        ...
        while self.apple in snake_pixels:
            ...
            self.super_apple = randint(0,100) <= SUPER_APPLE_CHANCE
            x = randint(0, WIDTH - 1)
    ...
    def draw(self):
        ...
        if not self.death:
            ...
            self.draw_snake()
            self.draw_score()
            color_apple = COL_APPLE if not self.super_apple else COL_SUPER_APPLE
            pyxel.pset(self.apple.x, self.apple.y, col=color_apple)
